class YouTubeCaptionExtension {
  ...
  async loadCaptions(targetLanguageCode = 'en') {
    ...
    try {
      ...
      if (!nativeLoaded) {
        ...
        if (apiKey) {
          playerResponse = await this.getPlayerResponseAndroid(videoId, apiKey, visitorData);
          if (playerResponse && playerResponse.captions) {
            nativeLoaded = await this.fetchAndRenderNativeCaptions(playerResponse, targetLanguageCode);
          }
        }
      ...
      // --- NEW LOGIC: Check if we only have auto-generated subtitles ---
      const hasOnlyAutoGenerated = this.availableCaptionTracks && 
                                   this.availableCaptionTracks.length > 0 && 
                                   this.availableCaptionTracks.every(track => track.kind === 'asr' || track.languageCode === 'en-auto');

      // If no native subs OR only auto-generated subs, check GitHub for manual ones
      if (!nativeLoaded || hasOnlyAutoGenerated) {
        console.log('🔄 Checking GitHub for manual subtitles (Native is missing or auto-generated)...');
        ...
        if (fallbackCaptions && fallbackCaptions.length > 0) {
          ...
          console.log(`✅ Loaded ${fallbackCaptions.length} captions from GitHub`);
          this.isLoading = false;
          return;
        }
      }
   ...
   */
  async getPlayerResponseAndroid(videoId, apiKey, visitorData) {
    console.log('📱 Attempting ANDROID client backup method...');
    const playerUrl = `https://www.youtube.com/youtubei/v1/player?key=${apiKey}`;
    const payload = {
      context: {
        client: {
  ...
  populateLanguageDropdown(captionTracks, selectedLanguageCode) {
    ...
    if (selectedLanguageCode === 'custom') {
      const option = document.createElement('option');
      option.value = 'custom';
      option.textContent = `Custom (${this.currentCustomFileName || 'GitHub'})`;
      option.selected = true;
      languageSelect.appendChild(option);
    }
  }
